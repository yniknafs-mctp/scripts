...
def plotter(amp, null, normal, name, output_file, log_in = False):
    big_list = []
    
    for x in amp:
        big_list.append([x, 'a', 'r'])
    for x in null:
        big_list.append([x, 'b', 'y'])
    for x in normal:
    ...
    MIN_CONSTANT = 1e-3
    expression_list = np.array(expression_list) + MIN_CONSTANT
    plt.figure(figsize=(10,5))
    p1 = plt.bar(ind, expression_list, width, color=color_list, edgecolor='none', log=log_in)
    p2 = plt.bar([0], [MIN_CONSTANT], color='r', edgecolor='none', log=log_in)
    p3 = plt.bar([0], [MIN_CONSTANT], color='y', edgecolor='none', log=log_in)
    p4 = plt.bar([0], [MIN_CONSTANT], color='b', edgecolor='none', log=log_in)
    ...
    plt.xlabel('Total Samples (N=%d), CNV_samples (N=%d)' % (len(expression_list), len(amp)))
    plt.xlim((0,1.01*len(expression_list)))
    plt.ylim((0.95*min(expression_list),1.05*max(expression_list)))
    #plt.show()
    plt.legend((p2[0], p3[0], p4[0]), ('CNV', 'Null', 'Normal'), loc=2, prop={'size':'small'})
