...
# create expression file
    def expression(pt_list, transcript_list, type):
        ...
        phenos_file_dir = os.path.join(args.output_dir, args.tumor_type + "_" + type + '.phenos.tsv')
        return expression_file_dir, phenos_file_dir
    amp_tsv_file, amp_phenos = expression(amp_pt_dir, amp_transcript_list, "amp")
    del_tsv_file, del_phenos = expression(del_pt_dir, del_transcript_list, "del")
...
#import data for normal tissues/i.e. load phenos file to start
    ...
    with open(args.phenos_file, 'r') as f:
        ...
        for row in reader: 
            ...
            if type.strip() == args.tumor_type and int(progression) == 11: 
                ...
                normal_count += 1
    normal_ptIDs.close()    
    amp_norm_tsv_file, amp_norm_pheno_file = expression(normal_ptIDs_dir, amp_transcript_list, 'amp_norm')
    del_norm_tsv_file , del_norm_pheno_file= expression(normal_ptIDs_dir, del_transcript_list, 'del_norm')
    ...
    def tsv_collapser(tsv_file):
        #print tsv_file
        #print os.path.dirname(tsv_file)
        #print os.path.basename(tsv_file)
    ...
    def tsv_processor(tsv_file, norm_dict, norm_expressions_dict, peak_dict, id_dict, type):
        ...
        with open(tsv_file, 'r') as f:
            ...
            if args.plot != 0:
                header_fields_copy[13:13] = ['cancer_type', 
                                             'peakID', 
                                             'peak_type', 
                                             'pos_mean', 
                                             'pos_count', 
                                             ...
                                             'normal_mean', 
                                             'normal_count',  
                                             'pos_null_fold_change', 
                                             'pos_null_p value', 
                                             'pos_normal_fold change', 
                                             'pos_normal_p_value', 
                                             'cancer_normal_fold_change', 
                                             'cancer_normal_p_value',
                                             'nes', 
                                             'nes_pval',
                                             'log_nes',
                                             'log_nes_pval',
                                             ...
                                             'ssea_hyperlink',
                                             'ssea_log_hyperlink', 
                                             'hyperlink_antecedent', 
                                             'log_hyperlink_antecedent', 
                                             'ssea_hyperlink_antecedent',
                                             'ssea_log_hyperlink_antecedent'] 
            else:
                header_fields_copy[13:13] = ['cancer_type', 
                                             'peakID', 
                                             'peak_type', 
            ...
            for row in reader:
                ...
                for x in id_dict[peak][0]:
                    if float(row[header_fields.index(x)])>=0:
                ...
                ssea_set = SampleSet(name="test", desc="test", value=set(id_dict[peak][0])) 
                log_results = ssea_run(ssea_ids, ssea_weights, [ssea_set], weight_method_miss="log", weight_method_hit="log")
                log_nes = log_results[0].nes
                log_pval = log_results[0].pval
                log_plot = log_results[0].plot()
                log_ssea_plot_dir = os.path.abspath(os.path.join(plot_dir, transcript + '_log_ssea.png'))
                log_plot.savefig(log_ssea_plot_dir)
                
                
                results = ssea_run(ssea_ids, ssea_weights, [ssea_set], weight_method_miss="unweighted", weight_method_hit="unweighted")
                nes = results[0].nes
                pval = results[0].pval
                plot = results[0].plot()
                ssea_plot_dir = os.path.abspath(os.path.join(plot_dir, transcript + '_ssea.png'))
                plot.savefig(ssea_plot_dir)
                ...
                if args.plot != 0:
                    ...
                    plot_hyperlink = '=HYPERLINK("file://"&AL' + str(i) + '; \"non-log\"'
                    plot_test.plotter(pos_expr, null_expr, norm_expressions_dict[transcript], transcript + " log", plot_file_log_dir, True)
                    plot_log_hyperlink = '=HYPERLINK("file://"&AM' + str(i) + '; \"log\"'
                    ssea_hyperlink = '=HYPERLINK("file://"&AN' + str(i) + '; \"ssea\"'
                    ssea_log_hyperlink = '=HYPERLINK("file://"&AO' + str(i) + '; \"log_ssea\"'
                    
                    
                    row_clip = row[:]
                    row_clip[13:13] = [args.tumor_type, 
                                       peak, 
                                       peak_type, 
                                       pos_mean, 
                                       pos_count, 
    ...
    sets_dir = os.path.join(folder, 'sets', args.tumor_type)
    if not os.path.exists(os.path.dirname(sets_dir)):
        os.mkdir(os.path.dirname(sets_dir))
    if not os.path.exists(sets_dir):
        os.mkdir(sets_dir)
        
    def peak_processor(lesion_file, ncrna_count_dict, protein_count_dict, genes_in_peak_dict, id_dict, type):
        ...
        with open(lesion_file, 'r') as f:
            ...
            for row in reader:
                ...
                set_file_dir = os.path.join(sets_dir, args.tumor_type + '_' + peakname + '.smx')
                file = open(set_file_dir, 'w')
                file.write('>' + args.tumor_type + '_' + peakname)
                file.write('\n')
                file.write('Pts with CNV for ' + peakname + " in " + args.tumor_type)
                file.write('\n')
                for pt in id_dict[peakname][0]:
                    file.write(pt)
                    file.write('\n')
                file.close()
    ...
    peak_processor(amp_lesion_dest, amp_ncrna_count_dict, amp_protein_count_dict, amp_genes_in_peak_dict, amp_id_dict, "amp")
    peak_processor(del_lesion_dest, del_ncrna_count_dict, del_protein_count_dict, del_genes_in_peak_dict, del_id_dict, "del")

#     
    p = subprocess.call(['rm', amp_lesion_dest, del_lesion_dest, amp_bedfile, del_bedfile, amp_peak_file_dest, del_peak_file_dest, 
                         amp_pt_dir, del_pt_dir, amp_transcript_list, del_transcript_list, amp_tsv_file, del_tsv_file,
                         amp_phenos, del_phenos, amp_norm_tsv_file, del_norm_tsv_file, amp_norm_pheno_file, del_norm_pheno_file,
                         DNA_id_file_dir, amp_gene_tsv_file, del_gene_tsv_file, amp_norm_gene_tsv_file, del_norm_gene_tsv_file, normal_ptIDs_dir])    
