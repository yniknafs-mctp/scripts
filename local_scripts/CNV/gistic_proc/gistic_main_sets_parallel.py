...
#indices for the ssea dict output
dict_es_in = 0
dict_global_q_in = 1
dict_fdr_in = 2
dict_nes_in = 3
...
def ssea_config(output_dir = "SSEA_%s" % (timestamp()), title = 'myssea'):
    config = Config()
...
def run_ssea(cnv_mat, null_mat, cnv_libs, null_libs, peakID, trans, output_dir, sets):
    #prepare to run SSEA
    ...
    weight_vec_iter = WeightVector.from_data(rownames=trans,
                                     ...
                                     na_value=-1)
    config = ssea_config(output_dir, peakID)
    

    
    # run SSEA
    report_file = ssea_main(weight_vec_iter, sets, config)
    
    out_dict = collections.defaultdict(lambda: [])
    trans_list = []
    # parse output
    for rowdict in Report.parse(report_file):
        set = rowdict['sample_set_name']
        trans_name = rowdict['name']
        tuple = (trans_name, set)
        es = rowdict['es']
        nes = rowdict['nes']
        fdr = rowdict['fdr_q_value']
        g_nes = rowdict['global_nes']
        global_qval = rowdict['global_fdr_q_value']
        available_fields = rowdict.keys()
        out_dict[tuple] = [es, global_qval, fdr, nes, g_nes]
...
def main():
    ...
    for peak in lesions[0:2]:
        ...
        #run SSEA
        ssea_dict = run_ssea(cnv_mat, null_mat, cnv_libs, null_libs, peakID, trans, peakID, sample_sets)
        ...
        for x in xrange(len(trans)):
            transcript = trans[x]
            tuple_key = (transcript, peakID) 
            #make sure SSEA was actually run on transcript
            if ssea_dict[tuple_key] != []:
                #define variables to print in transcript_file that do change
                #from transcript to transcript
                es = ssea_dict[tuple_key][dict_es_in]
                qval = ssea_dict[tuple_key][dict_fdr_in]
                nes = ssea_dict[tuple_key][dict_nes_in]
                g_nes = ssea_dict[tuple_key][dict_g_nes_in]
                g_qval = ssea_dict[tuple_key][dict_global_q_in]
